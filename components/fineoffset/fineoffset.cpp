...
void IRAM_ATTR SignalProcessor::process_edge(uint32_t pulse) {
    // Note: Timing and noise filtering is handled by the caller (original ISR)
    // We only process the signal logic with the pre-calculated pulse duration

    uint8_t wh2_flags = 0x00;
    bool wh2_accept_flag = false;

    // Signal detection state machine (exactly as original)
    switch (sampling_state_) {
        case 0:  // waiting
            if (is_hi_pulse(pulse)) {
                wh2_flags = signal_flags::GOT_PULSE | signal_flags::LOGIC_HI;
                sampling_state_ = 1;
            } else if (is_low_pulse(pulse)) {
                wh2_flags = signal_flags::GOT_PULSE;  // logic low
            } else {
                sampling_state_ = 0;
            }
            break;
    ...
    if (wh2_flags) {
        // Acquire preamble (exactly as original)
        if (packet_state_ == 1) {
            // Shift history right and store new value
            history_ <<= 1;
            // Store a 1 if required (right shift alone will store a 0)
            if (wh2_flags & signal_flags::LOGIC_HI) {
                history_ |= 0x01;
            }

            // Check if we have a valid start of frame (xxxxx110)
            if ((history_ & protocol::PREAMBLE_MASK) == protocol::PREAMBLE_PATTERN) {
                // Clear packet and counters
                packet_no_ = 0;
                bit_no_ = 1;  // Start at 1 because only need to acquire 7 bits for first packet byte
        ...
        // Acquire packet (exactly as original)
        else if (packet_state_ == 2) {
            packet_buffer_[packet_no_] <<= 1;
            if (wh2_flags & signal_flags::LOGIC_HI) {
                packet_buffer_[packet_no_] |= 0x01;
            }
            bit_no_++;
            if (bit_no_ > 7) {
                bit_no_ = 0;
...
FineOffsetStore::FineOffsetStore(FineOffsetComponent* parent)
    : parent_(parent), signal_processor_(this), wh2_flags_{0}, packet_state_{0} {}
...
void IRAM_ATTR FineOffsetStore::intr_cb(FineOffsetStore* self) {
    ...
    // VALIDATION: Run new SignalProcessor with same pulse timing
    self->signal_processor_.process_edge(pulse);
    ...
    switch (sampling_state) {
        case 0:  // waiting
            ...
            if (pulse >= protocol::HI_PULSE_MIN && pulse <= protocol::HI_PULSE_MAX) {
                wh2_flags = signal_flags::GOT_PULSE | signal_flags::LOGIC_HI;
                sampling_state = 1;
            } else if (pulse >= protocol::LOW_PULSE_MIN && pulse <= protocol::LOW_PULSE_MAX) {
                wh2_flags = signal_flags::GOT_PULSE;  // logic low
            } else {
                sampling_state = 0;
            }
            break;
        case 1:  // observe 1ms of idle time
            if (pulse > protocol::IDLE_TIMEOUT) {
                sampling_state = 0;
                wh2_packet_state = 1;
            } else if (pulse >= protocol::IDLE_PERIOD_MIN) {
                sampling_state = 0;
            } else {
                sampling_state = 0;
                wh2_packet_state = 1;
    ...
    // VALIDATION: Compare original static state with new SignalProcessor state
    if (!self->signal_processor_.validate_against_original(sampling_state, wh2_packet_state, packet_no, bit_no, history,
                                                           wh2_packet)) {
        // States don't match - log error for debugging
        ESP_LOGE("fineoffset.validation", "SignalProcessor state mismatch detected!");
        ESP_LOGE("fineoffset.validation", "Original: samp=%d pkt=%d pktno=%d bitno=%d hist=0x%02X", sampling_state,
                 wh2_packet_state, packet_no, bit_no, history);
    }
